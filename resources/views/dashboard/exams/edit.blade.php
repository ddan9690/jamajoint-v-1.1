...
<div class="max-w-4xl mx-auto">
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-slate-800">Edit Exam: {{ $exam->name }}</h1>
    </div>

    {{-- Form action now passes both exam ID and slug to match the route requirements --}}
    <form action="{{ route('exams.update', ['exam' => $exam->id, 'slug' => $exam->slug]) }}" 
          method="POST" 
          class="bg-white p-6 rounded-lg shadow-sm border border-slate-200 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="md:col-span-2">
                ...
                <input type="text" 
                       ...
                       placeholder="e.g. Spark English Joint Exam" 
                       class="w-full border border-slate-300 rounded-lg p-2.5 outline-none focus:ring-2 focus:ring-blue-500" 
                       required>
            </div>
            ...
            <div>
                ...
                <select name="form_id" class="w-full border border-slate-300 rounded-lg p-2.5 outline-none" required>
                    @foreach($forms as $f) 
                        <option value="{{$f->id}}" {{ $exam->form_id == $f->id ? 'selected' : '' }}>{{$f->name}}</option> 
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <select name="grading_system_id" class="w-full border border-slate-300 rounded-lg p-2.5 outline-none" required>
                    @foreach($gradingSystems as $g) 
                        <option value="{{$g->id}}" {{ $exam->grading_system_id == $g->id ? 'selected' : '' }}>{{$g->name}}</option> 
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <select name="mark_submission_mode" class="w-full border border-slate-300 rounded-lg p-2.5 outline-none" required>
                    <option value="admins" {{ $exam->mark_submission_mode == 'admins' ? 'selected' : '' }}>Exam Admins</option>
                    <option value="teachers" {{ $exam->mark_submission_mode == 'teachers' ? 'selected' : '' }}>Participating School Teachers</option>
                </select>
            </div>
        </div>
